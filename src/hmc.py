...
class HMCFieldTheory1D(FieldTheory1D):
    ...
    def compute_force(self, field: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.N):
            if self.boundary_conditions == 'periodic':
                left = (i - 1) % self.N
                right = (i + 1) % self.N
                # Contribution from (φ_i - φ_{i-1})² and (φ_{i+1} - φ_i)²
                force[i] += (field[i] - field[left]) - (field[right] - field[i])
            else:
    ...
    def run_hmc_simulation(self, n_trajectories: int, step_size: float = 0.1,
                          n_md_steps: int = 10, burn_in: int = 1000,
